...
for word, count, rank, product, POS in words_ranking:
    if i >= 50:
        break
    print(f"{word}: {count} (rank: {rank}, product: {product}, POS: {POS})")
    i += 1
part_time = time.time()
...
part_time = time.time()
print(f"Time to get first 2000 words: {part_time - start_time:.2f} seconds")
G = nx.Graph()
...
for (word, POS), degree in top_words_by_connections:
    if i >= 200:
        break
    print(f"{word} (POS: {POS}): {degree} connections")
    i += 1
